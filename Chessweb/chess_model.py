...
MODEL_PATH = "C:\HocML\2526II_AIT2004_4-CSAI-Project-ChessAI\Chessweb\best_model.pt"
CHANNELS   = 64
RES_BLOCKS = 6
ATTN_HEADS = 4
ATTN_EVERY = 3
...
def decode_move_canonical(move_idx: int, board: chess.Board,
                           flip: bool) -> Optional[chess.Move]:
    from_sq = move_idx // 64
    to_sq   = move_idx % 64
    if flip:
...
class MCTSEngine:
    ...
    def search(self, board, last_move, num_simulations=200, add_noise=False):
        ...
        if board_copy.is_game_over():
            return root
        priors, _ = self._evaluate_leaf(board_copy, last_move)
        self._do_expand(root, board_copy, priors)
        root.visit_count = 1
        if add_noise:
